...
w_ans = zeros(runs,4);
f = zeros(runs,iter+1);
gnorm = zeros(runs,iter+1);
runtime = zeros(runs,iter+1);
for r = 1 : runs
    [w_temp,f_temp,gnorm_temp,runtime_temp] = MyLBFGS(fun,gfun,w,Y,iter,N_g,N_h,M);
    w_ans(r,:) = w_temp';
    f(r,:) = f_temp';
    gnorm(r,:) = gnorm_temp';
...
lighting gouraud
alpha(0.3);
filename = [folder,'bfgs_iter',num2str(iter), '_M',num2str(M), ...
    '_batch',num2str(N_g),'_',num2str(N_h),postfix,'.fig'];
saveas(gcf,filename)
%%
figure;
hold on;
...
filename = [folder,'bfgs_f_iter',num2str(iter), '_M',num2str(M),...
    '_batch',num2str(N_g),'_',num2str(N_h),postfix,'.png'];
saveas(gcf,filename)
%%
figure;
hold on;
...
filename = [folder,'bfgs_gnorm_iter',num2str(iter), '_M',num2str(M),...
    '_batch',num2str(N_g),'_',num2str(N_h),postfix,'.png'];
saveas(gcf,filename)
%%
figure;
...
ylabel('f','Fontsize',fsz);
filename = [folder,'bfgs_f_runtime_iter',num2str(iter), '_M',num2str(M),...
    '_batch',num2str(N_g),'_',num2str(N_h),postfix,'.png'];
saveas(gcf,filename)
end
%%
