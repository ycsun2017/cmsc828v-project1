...
% [w,f,gnorm] = SINewton(fun,gfun,Hvec,Y,w,1e3); % unconstrained problem
[w,f,gnorm] = SINewton(fun,gfun,Hvec,Y,w,1e3); % initialization of SVM
e = ones(n,1);
xi = max(e-Y*w,0);
x = [w;xi];
% active = A*x - b;
...
% fun = @(I,Y,w)fun0(I,Y,w,lam);
% gfun = @(I,Y,w)gfun0(I,Y,w,lam);
% [w,f,gnorm] = SGD(w,Y,fun,gfun,1e3,0.01,64);
